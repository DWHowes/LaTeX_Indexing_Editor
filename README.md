...
You might also occasionally see `xfailed` in the summary line, e.g. `150 passed, 1 xfailed`. That's not a failure — it marks a small number of known, deliberately-tracked issues that are expected to fail until someone specifically fixes them. Don't worry about these unless you're the one doing that fix.
