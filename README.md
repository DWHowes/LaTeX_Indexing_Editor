...
You might also occasionally see `xfailed` in the summary line (for example: `242 passed, 1 xfailed`). That's not a failure — it marks a known, deliberately-tracked issue that's expected to fail until someone specifically fixes it. Don't worry about these unless you're the one doing that fix.
