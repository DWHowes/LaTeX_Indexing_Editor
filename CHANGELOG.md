...
`probes/probe_core_wiring.py` reported it as the one core module that reaches
nothing here and is not declared. That is right: it records where each shipped
table came from, and its readers are its guard test and the name-inversion
article's Appendix B generator. No application shows a table's source, so it is
declared rather than wired. The probe is clean.
...
this application's table parses with. A party that ran into the line above took
that entry's tail into its name **and cost that entry its place in the table**,
because the two spans overlapped and the longer won. Also read now: a party
with a lower-case *and others*, a name whose particles stack (*Van der Linde*)
or begins with one (*de Souza*). Scope:
...
`bookindexcore.authorities.forms`, which this application's table parses with.
A McGill book gains the articles cited with an issue (`19:1`) or a journal in
full, and 23 articles that printed a wrong page are corrected. Scope:
